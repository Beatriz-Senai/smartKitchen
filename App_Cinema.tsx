...
export default function App() {
  ...
  async function gerarCinema() {
    ...
    const prompt = `Sugira obras cinematográficas de gênero ${pref1}, que passa sensações de ${pref2}, que seja um filme em ${pref3} e que tenha no streaming ${pref4}. `;
    ...
    try {
      ...
      setCinema(result.response.text());
    } catch (error) {
      console.error(error);
    } finally {
      setLoad(false);
  ...
  return (
    <View style={ESTILOS.container}>
      ...
      <View style={ESTILOS.form}>
        ...
        />
        <TextInput
          placeholder="Sensações ao assistir"
          style={ESTILOS.input}
          value={pref2}
          onChangeText={(texto) => setPref2(texto)}
        />
        <TextInput
          placeholder="Você prefere filmes animados ou live-action?"
          style={ESTILOS.input}
          value={pref3}
          onChangeText={(texto) => setPref3(texto)}
        />
        <TextInput
          placeholder="Streaming que prefere assistir"
          style={ESTILOS.input}
          value={pref4}
      ...
      <TouchableOpacity style={ESTILOS.button} onPress={gerarCinema}>
        <Text style={ESTILOS.buttonText}>Recomendar obras</Text>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 24, marginTop: 4, }} style={ESTILOS.containerScroll} showsVerticalScrollIndicator={false} >
        {load && (
          <View style={ESTILOS.content}>
            <Text style={ESTILOS.title}>Procurando indicações de obras cinematográficas...</Text>
        ...
        {cinema && (
          <View style={ESTILOS.content}>
            ...
            <Text style={{ lineHeight: 24, color:'#fff' }}>{cinema}</Text>
          </View>
        )}
      </ScrollView>
    </View>
